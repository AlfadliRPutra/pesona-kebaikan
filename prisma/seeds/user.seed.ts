...
import { Role, NotificationType, VerifiedAs, VerificationStatus } from "@/generated/prisma";
...
export async function seedUsers() {
	...
	if (usersAll.length > 0) {
		const accountsData = usersAll.slice(0, Math.min(20, usersAll.length)).map((u) => ({
			userId: u.id,
			type: "oauth",
			provider: "google",
		...
		const sessionsData = usersAll.slice(0, Math.min(20, usersAll.length)).map((u) => ({
		...
		const vtData = usersAll.slice(0, Math.min(10, usersAll.length)).map((u) => ({
			identifier: u.email,
		...
		const authnData = usersAll.slice(0, Math.min(5, usersAll.length)).map((u) => ({
		...
		if (loginCount === 0) {
			const activities = usersAll.slice(0, Math.min(50, usersAll.length)).map((u) => ({
				userId: u.id,
		...
		if (postCount === 0) {
			const postsData = usersAll.slice(0, Math.min(10, usersAll.length)).map((u) => ({
		...
		if (notifCount === 0) {
			...
			const notifsData = usersAll.slice(0, Math.min(50, usersAll.length)).map((u) => ({
		...
		if (otpCount === 0) {
			const otpsData = usersAll.slice(0, Math.min(10, usersAll.length)).map((u) => ({
		...
		if (verReqCount === 0) {
			const reqsData = usersAll.slice(0, Math.min(20, usersAll.length)).map((u, idx) => {
				const isOrg = idx % 2 === 1;
				return {
					userId: u.id,
					type: isOrg ? VerifiedAs.organization : VerifiedAs.personal,
					status: VerificationStatus.PENDING,
					ktpNumber: isOrg ? undefined : faker.number.int({ min: 1000000000000000, max: 9999999999999999 }).toString(),
					ktpName: isOrg ? undefined : u.name || undefined,
					ktpPhotoUrl: isOrg ? undefined : "https://picsum.photos/600/400?ktp",
					selfieUrl: isOrg ? undefined : "https://picsum.photos/600/400?selfie",
					organizationName: isOrg ? faker.company.name() : undefined,
					organizationDocUrl: isOrg ? "https://picsum.photos/600/400?doc" : undefined,
				};
			});
