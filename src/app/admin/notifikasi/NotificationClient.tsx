...
import { createNotification, deleteNotification, broadcastNotification } from '@/actions/notification';
...
export default function NotificationClient({ initialNotifications, totalPages, currentPage }: NotificationClientProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (targetMode === 'BROADCAST') {
        ...
        result = await createNotification(selectedUser.id, title, message, type);
      }
            ...
            {initialNotifications.length === 0 ? (
              ...
              initialNotifications.map((notif) => (
                <TableRow key={notif.id} hover>
                  ...
                  <TableCell>
                    {notif.user ? notif.user.name || notif.user.email : 'Unknown'}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell align="right">
                    ...
                    >
                      {deleteLoading === notif.id ? <CircularProgress size={20} /> : <DeleteIcon fontSize="small" />}
                    </IconButton>
                  </TableCell>
                </TableRow>
              ))
        ...
        <DialogContent dividers>
          <Stack spacing={3} sx={{ pt: 1 }}>
            ...
            {targetMode === 'SINGLE' && (
              <Autocomplete
                options={userOptions}
                getOptionLabel={(option) => `${option.name || 'No Name'} (${option.email})`}
