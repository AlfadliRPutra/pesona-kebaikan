...
const ImageUpload = ({ label, value, onChange, height = 200, id }: { label: string, value: string, onChange: (url: string) => void, height?: number, id?: string }) => {
    ...
    <Box sx={{ mb: 2 }}>
      ...
      >
        {value ? (
          ...
          <>
            {uploading ? <CircularProgress size={24} /> : <CloudUploadIcon sx={{ fontSize: 40, color: "#94a3b8", mb: 1 }} />}
            <Typography variant="caption" color="text.secondary">Klik untuk upload gambar</Typography>
          </>
        )}
        <input
          type="file"
      ...
      {value && (
        <Button size="small" color="error" onClick={(e) => { e.stopPropagation(); onChange(""); }} sx={{ mt: 1 }}>
          Hapus Gambar
        </Button>
      )}
    </Box>
...
export default function AdminTentangPage() {
  ...
  const handleOrgAdd = () => {
    setData(prev => ({
      ...prev,
      organization: [...prev.organization, { name: "", position: "", image: "" }]
    }));
  };
  ...
  const handleOrgChange = (index: number, field: keyof OrgMember, value: string) => {
  ...
  const handleLegalityChange = (index: number, field: keyof Legality, value: string) => {
    const newLegality = [...data.legality];
    newLegality[index] = { ...newLegality[index], [field]: value };
    setData(prev => ({ ...prev, legality: newLegality }));
  };

  const handleLegalityUpload = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files?.[0]) return;
    const formData = new FormData();
    formData.append("file", e.target.files[0]);
    try {
    ...
    <Box>
      ...
      <Box sx={{ mb: 4, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Box
            sx={{
              width: 48, height: 48, borderRadius: 3, bgcolor: "#e0f2fe",
      ...
      <Paper elevation={0} sx={{ borderRadius: 3, border: "1px solid #e2e8f0" }}>
          ...
          <Stack spacing={4}>
            <Box>
              ...
              <Alert severity="info" sx={{ mt: 1, borderRadius: 2 }}>
                Banner akan ditampilkan di bagian atas halaman publik. Pastikan rasio gambar landscape (misal 16:9 atau 21:9) untuk hasil terbaik.
              </Alert>
            </Box>

            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 600, mx: 'auto', width: '100%' }}>
            ...
            <Box sx={{ px: { xs: 1, md: 4 } }}>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Cerita Kami (About Us)</Typography>
              <RichTextEditor
                value={content}
                onChange={setContent}
        ...
        <CustomTabPanel value={tab} index={1}>
          <Stack spacing={4}>
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Visi</Typography>
              <RichTextEditor
                value={data.vision}
                onChange={(val) => setData(prev => ({ ...prev, vision: val }))}
                minHeight={200}
              />
            </Box>
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Misi</Typography>
              <RichTextEditor
                value={data.mission}
                onChange={(val) => setData(prev => ({ ...prev, mission: val }))}
                minHeight={200}
              />
            </Box>
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Tujuan</Typography>
              <RichTextEditor
                value={data.goals}
            ...
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Capaian & Prestasi</Typography>
              <RichTextEditor
                value={data.achievements}
                onChange={(val) => setData(prev => ({ ...prev, achievements: val }))}
                minHeight={200}
              />
            </Box>
          </Stack>
        ...
        <CustomTabPanel value={tab} index={3}>
          <Button startIcon={<AddIcon />} variant="outlined" onClick={handleOrgAdd} sx={{ mb: 3 }}>
            Tambah Anggota Tim
          </Button>
          <Grid container spacing={3}>
                ...
                <Card variant="outlined" sx={{ borderRadius: 2 }}>
                  <CardContent>
                    <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
                      <IconButton size="small" color="error" onClick={() => handleOrgRemove(index)}>
                    ...
                    <Stack spacing={2}>
                      ...
                      <TextField
                        ...
                        onChange={(e) => handleOrgChange(index, "position", e.target.value)}
                      />
                    </Stack>
                  </CardContent>
                </Card>
        ...
        <CustomTabPanel value={tab} index={4}>
          <Button startIcon={<AddIcon />} variant="outlined" onClick={handleLegalityAdd} sx={{ mb: 3 }}>
                    ...
                    <Grid item xs={12} md={5}>
                      <Box sx={{ display: "flex", gap: 1 }}>
                        <TextField
                          ...
                          label="URL File"
                          value={doc.url}
                          onChange={(e) => handleLegalityChange(index, "url", e.target.value)}
                        />
                        <Button
                          component="label"
                          variant="outlined"
                        ...
                        >
                          <CloudUploadIcon />
                          <input type="file" hidden onChange={(e) => handleLegalityUpload(index, e)} />
                        </Button>
                      </Box>
                    </Grid>
                  </Grid>
                </Box>
                <IconButton color="error" onClick={() => handleLegalityRemove(index)}>
                  <DeleteIcon />
                </IconButton>
              </Paper>
            ))}
