...
export default function AdminCampaignTable({
	...
	function getComparator(
		order: "asc" | "desc",
		orderBy: keyof AdminCampaignRow,
	) {
		return (a: AdminCampaignRow, b: AdminCampaignRow) => {
			let av: any = a[orderBy];
			let bv: any = b[orderBy];

			if (orderBy === "updatedAt") {
				av = parseDateSafe(String(a.updatedAt));
				bv = parseDateSafe(String(b.updatedAt));
			}
			...
			const sa = String(av || "");
			const sb = String(bv || "");
			const cmp = sa.localeCompare(sb, "id", { sensitivity: "base" });
	...
	const handleRequestSort = (property: keyof AdminCampaignRow) => {
	...
	const someSelected =
		selectedIds.some((id) => visibleIds.includes(id)) && !allSelected;
	return (
		<TableContainer
			component={Paper}
			sx={{
		...
		>
			...
			>
				<TableHead>
					<TableRow>
						<TableCell sx={{ width: 44 }} align="center">
							<Checkbox
								checked={allSelected}
								indeterminate={someSelected}
								onChange={(e) => onToggleAll(e.target.checked, visibleIds)}
								size="small"
							/>
						</TableCell>
						...
						<TableCell sx={{ fontWeight: 900 }} align="center">
							<TableSortLabel
								active={orderBy === "collected"}
								direction={orderBy === "collected" ? order : "asc"}
								onClick={() => handleRequestSort("collected")}
							>
								Dana
							</TableSortLabel>
						</TableCell>
						<TableCell sx={{ fontWeight: 900 }} align="center">
							<TableSortLabel
								active={orderBy === "donors"}
								direction={orderBy === "donors" ? order : "asc"}
								onClick={() => handleRequestSort("donors")}
							>
								Donatur
							</TableSortLabel>
						</TableCell>
						...
						<TableCell sx={{ fontWeight: 900, width: 60 }} align="right">
							Aksi
						</TableCell>
					</TableRow>
				</TableHead>
						...
						sortedRows.map((row) => {
							...
							return (
								<TableRow
									key={row.id}
									hover
									onClick={() => router.push(`/admin/campaign/${row.id}`)}
									sx={{ cursor: "pointer" }}
								>
									<TableCell
										align="center"
